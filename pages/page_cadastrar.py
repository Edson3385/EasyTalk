...
def main(page):
    ...
    def cadastrar(e):
        ...
        if nome.value == "" or login.value == "" or password.value == "" or confirm_password.value == "":
            page.banner.content = ft.Text("Ainda há campos obrigatórios a serem preenchidos. Tente novamente.", color=ft.colors.BLACK)
            show_banner(e)
            page.update()
        #Verifica se as senha coincidem 
        elif password.value != confirm_password.value:
            page.banner.content = ft.Text("As senhas não coincidem.", color=ft.colors.BLACK)
            show_banner(e)
            page.update()
        #Verifica se a senha possue o tamanho mínimo necessário
        elif password.value.__len__() < 6:
            page.banner.content = ft.Text("A senha precisa ter no mínimo seis caracteres.", color=ft.colors.BLACK)
            show_banner(e)
            page.update()
        else:
           try:
                ...
                page.banner.content = ft.Text("Tudo certo! Cadastro realizado com sucesso. Verifique sua conta a partir do link de confirmação enviado para você por e-mail.", color=ft.colors.BLACK)
                ...
                for usuario in usuarios.each():
                    ...
                    if usuario.val()['email'] == login.value:
                        ...
                        page.banner.content = ft.Text("O e-mail informado já possui um cadastro.", color=ft.colors.BLACK)
                        page.banner.actions = [ft.TextButton(content=ft.Text("Fazer login", color=ft.colors.BLACK), on_click=go_to_login)]
                        page.update()
                    else:
                        #Erro de Cadastro não Realizado
                        page.banner.leading = ft.Icon(ft.icons.ERROR_OUTLINE, color=ft.colors.RED, size=40)
                        page.banner.content = ft.Text("Erro ao cadastrar usuário, verifique as informações inseridas e tente novamente.", color=ft.colors.BLACK)
