    ...
    def click_login(e):
        ...
        try:
            ...
            if emailVerified:
                ...
                page.banner.content = ft.Text("Esse e-mail não foi verificado. Para entrar no EasyTalk é necessário confirmar sua conta a partir do link enviado para você por e-mail.", color=ft.colors.BLACK)
            ...
            password.value = ""
            page.banner.leading = ft.Icon(ft.icons.ERROR_OUTLINE, color=ft.colors.RED, size=40)
            page.banner.content = ft.Text("E-mail ou senha incorretos.", color=ft.colors.BLACK)
            page.banner.actions = [ft.TextButton(content=ft.Text("OK", color=ft.colors.BLACK), on_click=close_banner)]
            show_banner(e) 
            time.sleep(6)
            page.banner.open = False
        ...
        for usuario in usuarios.each():
            ...
            if usuario.val()['email'] == email:
                ...
                try:
                    ...
                    page.banner.content = ft.Text("E-mail de redefinição de senha enviado com sucesso.", color=ft.colors.BLACK)
                    page.banner.actions = [ft.TextButton(content=ft.Text("OK", color=ft.colors.BLACK), on_click=close_banner)]
                    ...
                    page.banner.leading = ft.Icon(ft.icons.ERROR_OUTLINE, color=ft.colors.RED, size=40)
                    page.banner.content = ft.Text("Falha ao enviar o e-mail de redefinição de senha.", color=ft.colors.BLACK)
                    page.banner.actions = [ft.TextButton(content=ft.Text("OK", color=ft.colors.BLACK), on_click=close_banner)]
